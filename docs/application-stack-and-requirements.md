...
| FR-009 | Users can generate a periodized draft training plan from race distance, training weeks, sessions per week, optional race day, selected program intensity, and recent activity history. | `apps/web/src/app/api/program/generate/route.ts`; `apps/web/src/app/coach/GoalForm.tsx`; `ProgramIntensitySchema`                                     | Must                 |
| FR-010 | AI-generated plans must pass governance and schema validation before persistence; physiological validation results are stored as draft warnings instead of blocking preview.           | `PlanGeneratorOutputSchema`; `rules.validatePlan`; plan generation route draft metadata                                                               | Must                 |
| FR-011 | If the first generated plan violates physiological rules, the system runs one corrective second iteration and stores any remaining rule violations as visible warnings.                | `apps/web/src/app/api/program/generate/route.ts`; warning display in `apps/web/src/app/coach/draft/page.tsx` and `apps/web/src/app/calendar/page.tsx` | Must                 |
| FR-012 | Users must explicitly accept a draft before it replaces an existing active plan; accepting archives prior active programs in one transaction.                                          | `apps/web/src/app/api/program/[id]/accept/route.ts`; `apps/web/src/app/coach/draft/DraftPlanActions.tsx`                                              | Must                 |
...
| NFR-003 | Security        | AI agent actions must fail closed when governance or schema validation fails; physiological rule violations must be captured as explicit warnings before user acceptance. | `govern()`, Zod schemas, deterministic `rules.validatePlan`, draft warning metadata, and accept flow ownership checks.       | Unit tests for governance and rules; API error handling review. |
